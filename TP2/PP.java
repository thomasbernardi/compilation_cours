...
class PPArrayGet extends PPExpr {
    ...
    UPPExpr toUPP(ArrayList<String> locals) {
        //TODO
        PPExpr addr = Util.toAddress(arr, index);
        return new UPPLoad(addr.toUPP(locals));
    }//toUPP
...
class PPArraySet extends PPInst {
    ...
    UPPInst toUPP(ArrayList<String> locals) {
        //TODO
        PPExpr addr = Util.toAddress(arr, index);
        return new UPPStore(addr.toUPP(locals), val.toUPP(locals));
    }//toUPP
...
class PPProc extends PPDef {
    ...
    UPPDef toUPP () {
        ...
        for (Pair<String, Type> element : args) {
            nargs.add(element.left);
        }
        for (Pair<String, Type> element : locals) {
            nlocals.add(element.left);
        }

        return new UPPProc(name, nargs, nlocals, code.toUPP(nlocals));
    }//toUPP
...
class Util {
    public static PPExpr toAddress(PPExpr arr, PPExpr index) {
        PPExpr offset = new PPMul(new PPCte(4), index);
        return new PPAdd(arr, offset);
    }
}
